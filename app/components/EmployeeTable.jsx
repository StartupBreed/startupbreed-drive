...
export default function EmployeeTable() {
  ...
  useEffect(() => {
    if (!employees.length) { setClosedCounts({}); return; }
    async function scan() {
      setScanning(true);
      ...
      const closed = allPositions.filter((p) => p.properties?.status === 'closed');
      const counts = {};
      employees.forEach((emp) => {
        counts[emp.id] = closed.filter(
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200 text-xs font-semibold text-gray-500 uppercase tracking-wide">
                ...
                <th className="text-left px-5 py-3">Positions Closed</th>
                ...
                <tr key={emp.id} className="hover:bg-gray-50 transition-colors group">
                  ...
                  <td className="px-5 py-4">
                    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-sm font-semibold ${
                      (closedCounts[emp.id] || 0) > 0 ? 'bg-indigo-50 text-indigo-700' : 'bg-gray-100 text-gray-400'
                    }`}>
                      {scanning ? '…' : closedCounts[emp.id] || 0}
                      <span className="font-normal text-xs">closed</span>
                    </span>
                  </td>
    ...
    <div>
      {[...Array(3)].map((_, i) => (
        <div key={i} className="px-5 py-4 border-b border-gray-100 flex items-center gap-4">
          ...
          <div className="h-6 bg-gray-100 rounded-full animate-pulse w-24 ml-auto" />
        </div>
      ))}
    </div>
  );
